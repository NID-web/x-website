...
**Layout**
- A page is **one grid**. Use `PageGrid` + `GridItem`. Do not nest per-section flex containers, or a second `PageGrid` inside a `GridItem` — either doubles the shell margin (caught once by measuring, see STAGE-0-NOTES §6). A section that must place a cell by row (a utility slot) is a `GridItem subgrid`: it shares the page's column tracks and adds no margin or gutter (STAGE-0-NOTES §33). `subgrid` composes with `span` rather than replacing it — a subgrid item still says how many tracks it takes, and they nest, which is how the feature card puts its photo and text on real columns (§40).
...
- Section separators are omitted on phones (1 column) only — the 768 boards draw them. A utility slot (the page's, or a section's links) sits in the last column of the title row at 3–4 columns and leaves the row below that.
- `--nid-grid-shell-width` is `1440px` — the shell's **cap**, not its width. Below 1440 the shell is fluid (viewport minus the page margin), so the token over-reports it; measure a real `[data-nid-shell]` element's rect instead (STAGE-0-NOTES §5, §19). It is declared once in `:root` and never in a media query.
