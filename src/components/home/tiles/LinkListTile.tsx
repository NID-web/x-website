      ...
      <ul className="relative mt-5 flex flex-col gap-4">
        {tile.links.map((link) => (
          <li key={link.labelKey}>
            <Link
              href={link.href}
              className="group inline-flex items-center gap-1.5 font-primary text-h6 uppercase text-text-primary no-underline transition-colors duration-150 ease-in-out hover:text-accent-primary"
            >
              {t(link.labelKey)}
            ...
            {link.metaKey && (
              <p className="mt-1.5 font-body text-caption text-text-tertiary">
                {t(link.metaKey)}
              </p>
            )}
          </li>
